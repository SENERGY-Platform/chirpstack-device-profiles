			...
			case 0:
				{
					...
					if (Ext == "0") {
						...
						decode.Ext_sensor = "Temperature Sensor";
						decode.TempC_TMP117 = parseFloat(
							((((bytes[7] << 24) >> 16) | bytes[8]) / 100).toFixed(2)
						);
					} else if (Ext == "4") {
						...
						decode.Ext_TempC_SHT = parseFloat(
							((((bytes[7] << 24) >> 16) | bytes[8]) / 100).toFixed(2)
						);
						decode.Ext_Hum_SHT = parseFloat(
							((((bytes[9] << 8) | bytes[10]) & 0xfff) / 10).toFixed(1)
						);
					} else if (Ext == "14") {
						decode.Work_mode = "PIR Sensor";
						decode.Exti_pin_level =
							bytes[7] & 0x01 ? "Activity" : "No activity";
			...
			case 1:
				{
					for (var i = 0; i < bytes.length; i = i + 11) {
						var da = datalog(i, bytes);
						if (i == "0") data.retransmission_message = da;
						else data.retransmission_message += da;
					}
				}
				{
					return data;
